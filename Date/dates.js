// const d=new Date();
// document.getElementById("demo").innerHTML=d;

// const d=new Date(2019,3,21,11,30,25);
// const d1 = new Date(2018, 15, 24, 10, 33, 30);
// document.getElementById("demo").innerHTML=d;
// document.getElementById("demo1").innerHTML=d1;

// const d=new Date(2019,3,21,11,30,25);
// const d1=new Date(2019,3,21,11,30);
// const d2=new Date(2019,3,21,11);
// const d3=new Date(2019,3,21);
// const d4=new Date(2019,3);
// const d5=new Date(2019);
// document.getElementById("demo").innerHTML=d;
// document.getElementById("demo1").innerHTML=d1;
// document.getElementById("demo2").innerHTML=d2;
// document.getElementById("demo3").innerHTML=d3;
// document.getElementById("demo4").innerHTML=d4;
// document.getElementById("demo5").innerHTML=d5;

// const d1= new Date("2015-03-25");
// const d2= new Date("2015-03-25T12:00:00Z");
// const d3= new Date("03/25/2015");
// const d4= new Date("2015-3-25");
// const d5= new Date("2015/03/25");
// const d6= new Date("25-03-2015");
// const d7= new Date("Mar 25 2015");
// const d8= new Date("25 Mar 2015");
// const d9= new Date("January 25 2015");
// const d10= new Date("Jan 25 2015");
// const d11= new Date("JANUARY, 25, 2015");